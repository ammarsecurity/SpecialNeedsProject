...
<strong>Error !</strong> Email already Exists, please try another Email... !</div>';
  ...
  <strong>Error !</strong> Something went Wrong !</div>';
   ...
   // Check Exist Email Address Method
  public function checkSpecialNeeds($child_name){
    $sql = "SELECT child_name from  tbl_children WHERE child_name = :child_name";
    $stmt = $this->db->pdo->prepare($sql);
  ...
  public function addSpecialNeedsRegistration($data){
    $child_name = $data['child_name'];
    $child_age = $data['child_age'];
    $child_special_type = $data['child_special_type'];
    $useId  = Session::get('id') ;
   

    $checkSpecial = $this->checkSpecialNeeds($child_name);

    if ($child_name == "" || $child_age == 0 || $child_special_type == "") {
      $msg = '<div class="alert alert-danger alert-dismissible mt-3" id="flash-msg">
<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
<strong>Error !</strong> Input fields must not be Empty !</div>';
        return $msg;
    }elseif($checkSpecial == TRUE) {
      $msg = '<div class="alert alert-danger alert-dismissible mt-3" id="flash-msg">
<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
<strong>Error !</strong> This Child already Exists, please try another Child... !</div>';
        return $msg;
    }else{

      $sql = "INSERT INTO tbl_children(child_name, child_age, child_special_type , add_by ) VALUES(:child_name, :child_age, :child_special_type , :useId )";
      $stmt = $this->db->pdo->prepare($sql);
      $stmt->bindValue(':child_name', $child_name);
      $stmt->bindValue(':child_age', $child_age);
      $stmt->bindValue(':child_special_type', $child_special_type);
      $stmt->bindValue(':useId', $useId);
      $result = $stmt->execute();
      if ($result) {
        $msg = '<div class="alert alert-success alert-dismissible mt-3" id="flash-msg">
  <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
  ...
  <strong>Error !</strong> Something went Wrong !</div>';
    ...
    public function checkSpecialTypeName($specialType_Name){
    $sql = "SELECT specialType_Name from  tal_specialtypes WHERE specialType_Name = :specialType_Name";
    $stmt = $this->db->pdo->prepare($sql);
    $stmt->bindValue(':specialType_Name', $specialType_Name);
     $stmt->execute();
    if ($stmt->rowCount()> 0) {
      return true;
    }else{
  ...
  public function addSpecialType($data){
    $SpecialTypeName = $data['SpecialTypeName'];
  
  
    $checkSpecialTypeName = $this->checkSpecialTypeName($SpecialTypeName);
...
<strong>Error !</strong> Input fields must not be Empty !</div>';
        return $msg;
    }elseif($checkSpecialTypeName == TRUE) {
      $msg = '<div class="alert alert-danger alert-dismissible mt-3" id="flash-msg">
<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
<strong>Error !</strong> This Special Type Name already Exists, please try another ... !</div>';
        return $msg;
    }else{

      $sql = "INSERT INTO tal_specialtypes(specialType_Name) VALUES(:SpecialTypeName)";
      $stmt = $this->db->pdo->prepare($sql);
      $stmt->bindValue(':SpecialTypeName', $SpecialTypeName);
   
      $result = $stmt->execute();
      if ($result) {
        $msg = '<div class="alert alert-success alert-dismissible mt-3" id="flash-msg">
  <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
  <strong>Success !</strong> Wow, you have Add Successfully !</div>';
          return $msg;
      }else{
        $msg = '<div class="alert alert-danger alert-dismissible mt-3" id="flash-msg">
  <a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
  ...
  public function selectAllchildren(){
    $sql = "SELECT * 
			FROM tbl_children tc
			INNER JOIN tbl_users tu
			 on tu.id = tc.add_by
			INNER JOIN tal_specialtypes ts
			 on ts.id = tc.child_special_type
			";
    $stmt = $this->db->pdo->prepare($sql);
    $stmt->execute();
    return $stmt->fetchAll(PDO::FETCH_OBJ);
    ...
    public function selectSpecialTypesData(){
    $sql = "SELECT * FROM tal_specialtypes ORDER BY id DESC";
    $stmt = $this->db->pdo->prepare($sql);
    $stmt->execute();
    return $stmt->fetchAll(PDO::FETCH_OBJ);
  }
  ...
  <strong>Error !</strong> Invalid email address !</div>';
	...
	public function deletechild($remove){
      $sql = "DELETE FROM tbl_children WHERE id = :id ";
      $stmt = $this->db->pdo->prepare($sql);
      $stmt->bindValue(':id', $remove);
        $result =$stmt->execute();
        if ($result) {
